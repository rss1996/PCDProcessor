    ...
    def fit(self, X_train, y_train, epochs=10, batch_size=32, validation_data=None, callbacks=None):
        ...
        for epoch in range(epochs):
            ...
            for step, (inputs, labels) in enumerate(train_dataset):
                ...
                if step % 1 == 0:
                    print(f"Step {step}/{len(train_dataset)} - Loss: {metrics['loss']:.4f}, Accuracy: {metrics['accuracy']:.4f}, precision: {metrics['precision']:.4f},recall: {metrics['recall']:.4f},labels_p: {metrics['labels_p']},tp: {metrics['tp']},fp: {metrics['fp']}")
...
# 自定义加权二元交叉熵损失函数
def weighted_binary_crossentropy(y_true, y_pred, weight_pos=100, weight_neg=1):
    # 计算二元交叉熵损失
