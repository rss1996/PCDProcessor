...
class SetAbstraction(layers.Layer):
    ...
    def get_ball_query(self, point_cloud, radius, nsample):
        ...
        if batch_size is None:
            raise ValueError("batch_size cannot be None")  # 确保 batch_size 是有效的
...
class PointNetPlusPlus(tf.keras.Model):
    ...
    def custom_batch_sampler(self, dataset, batch_size, pos_ratio=0.9):
        # 使用 map 对每个样本的标签进行过滤
        def filter_pos_neg(x, y):
            # 过滤出正样本和负样本
            is_pos = tf.equal(y, 1)  # 对于正样本
            is_neg = tf.equal(y, 0)  # 对于负样本
            return x, is_pos, is_neg

        # 应用过滤函数
        dataset = dataset.map(filter_pos_neg)

        # 分别提取正样本和负样本
        pos_data = dataset.filter(lambda x, is_pos, is_neg: is_pos)
        neg_data = dataset.filter(lambda x, is_pos, is_neg: is_neg)



        pos_batch_size = int(batch_size * pos_ratio)
        neg_batch_size = batch_size - pos_batch_size

        pos_batch = pos_data.batch(pos_batch_size)
        neg_batch = neg_data.batch(neg_batch_size)

        return pos_batch.concatenate(neg_batch)
    ...
    def fit(self, X_train, y_train, epochs=10, batch_size=32, validation_data=None, callbacks=None):
        ...
        train_dataset = self.custom_batch_sampler(train_dataset, batch_size=batch_size, pos_ratio=0.5).batch(batch_size)
        ...
        for epoch in range(epochs):
            ...
            for step, (inputs, labels) in enumerate(train_dataset):
                ...
                if step % 1 == 0:
                    print(f"Step {step}/{len(train_dataset)} - Loss: {metrics['loss']:.8f}, Accuracy: {metrics['accuracy']:.4f}, precision: {metrics['precision']:.4f},recall: {metrics['recall']:.4f},labels_p: {metrics['labels_p']},tp: {metrics['tp']},fp: {metrics['fp']},count: {metrics['count']}")
            ...
            print(f"Train Loss: {train_loss:.8f}, Train Accuracy: {train_accuracy:.4f}, Train Precision: {train_precision:.4f}, Train Recall: {train_recall:.4f},labels_p: {train_labels_p},tp: {train_tp},fp: {train_fp}")
            ...
            if val_dataset is not None:
                ...
                print(f"Validation Loss: {val_loss:.8f}, Validation Accuracy: {val_accuracy:.4f}, Validation Precision: {val_precision:.4f}, Validation Recall: {val_recall:.4f},labels_p: {val_labels_p},tp: {val_tp},fp: {val_fp}")
            else:
...
# 自定义加权二元交叉熵损失函数
def weighted_binary_crossentropy(y_true, y_pred, weight_pos=1, weight_neg=2):
    # 计算二元交叉熵损失
